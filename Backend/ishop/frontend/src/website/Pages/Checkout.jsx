    ...
    axios.post(API_BASE_URL + "/order/place-order", {
      ...
      (response) => {
        if (response.data.status == 1) {
          ...
          if (paymentMode == 0) {
            navigator("/thank-you/" + response.data.orderId);

          } else {
            handlePayment(response.data.order.order_id, response.data.order.razorpay_order_id)
          }
          ...
          <div className="mb-8">
            ...
            {user?.shipping_address?.map((address, index) => (
              <div
                ...
                className={`p-4 border rounded-lg mb-4 cursor-pointer ${selectedAddress === index ? "border-teal-500 bg-blue-50" : "border-gray-300"
                  }`}
              >
            ...
            <div className="w-[100px] text-center p-2 bg-teal-500 border  rounded-md">+</div>
          </div>
            ...
            <div className="flex gap-4">
              ...
              <button
                ...
                className={`flex-1 py-3 text-center rounded-lg font-medium border ${paymentMode === 1
                  ? "bg-teal-500 text-white border-blue-500"
                  : "bg-teal-50 text-gray-700 border-gray-300"
                  }`}
              >
                Online Payment
              </button>
        ...
        <div className="w-full h-[400px] lg:w-1/3 bg-white shadow-lg rounded-lg p-6">
          ...
          <div className="p-4 bg-gray-50 border rounded-lg">
            <div className="flex justify-between mb-2">
              <p>Total Amount:</p>
              <p className="font-medium">{formatPriceINR(cart.originalTotal)}</p>
            </div>
            <div className="flex justify-between mb-2">
              <p>Discount:</p>
              <p className="text-green-600">{formatPriceINR(cart.originalTotal - cart.total)}</p>
            </div>
            <div className="flex justify-between mb-4">
              <p>Final Amount:</p>
              <p className="font-semibold text-lg">{formatPriceINR(cart.total)}</p>
            </div>
          </div>
          ...
          <button
            ...
            className="w-full mt-6 bg-teal-500 text-white py-3 rounded-lg font-semibold hover:bg-teal-600 transition"
          >
            Place Order
          </button>
        </div>
